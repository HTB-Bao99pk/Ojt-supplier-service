...
export default function StaffSidebar({ isCollapsed, onToggle }) {
        ...
        >
            ...
            <nav className={`flex-1 ${isCollapsed ? 'py-6 px-2' : 'py-6 px-3'} space-y-2 overflow-y-auto scrollbar-hide`}>
                {menuItems.map((item) => {
                    ...
                    const isActive = location.pathname === item.path || (!item.exact && location.pathname.startsWith(item.path));
