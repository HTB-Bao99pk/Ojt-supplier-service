...
import { getSupplierAuditLogs } from "../../api/supplierApi";
...
export default function SupplierAuditLogs() {
  ...
  const supplierName =
    logs.length > 0 && logs[0].newData
      ? JSON.parse(logs[0].newData).name
      : "Supplier";

  const renderAuditData = (dataString, type) => {
    if (!dataString) return <p className="text-gray-400 italic">N/A</p>;
    ...
    try {
      ...
      return (
        <div
          className={`p-3 rounded-lg text-xs ${type === "old" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
        >
      ...
      return (
        <div
          className={`p-3 rounded-lg text-xs whitespace-pre-wrap ${type === "old" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
        >
          {dataString}
        </div>
      );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Tracking all changes for: {supplierName}
          </p>
        </div>
      </div>
      ...
      <div className="relative border-l-2 border-gray-200 ml-4 space-y-8 pb-10">
        {loading ? (
          ...
          <div className="pl-8 text-gray-500">
            No history found for this supplier.
          </div>
        ) : (
          logs.map((log) => {
            ...
            return (
              <div key={log.id} className="relative pl-8">
                <span
                  className={`absolute-left-[9px] top-1 h-4 w-4 rounded-full border-4 border-white shadow-sm ${style.bg.split(" ")[1].replace("text", "bg")}`}
                ></span>
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex flex-wrap items-center justify-between gap-4 bg-gray-50/50 px-5 py-3 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${style.bg}`}
                      >
                        {style.icon}
                        {log.action}
                      </span>
